...
With _metrics_ metrics are exported from testserv and any handler that adds extra ones.
...
this, by default, only 1 in 10 queries will be monitored. See the syntax below on how to change this.
...
- `testserv_dns_requests_total{zone, network, family, flags}` - total query count.
- `testserv_dns_responses_total{zone, network, family, rcode}` - response per, among other things, the response code.
- `testserv_dns_request_duration_seconds{zone, network, family}` - duration to process each query.
- `testserv_dns_request_size_bytes{zone, network, family}` - size of the request in bytes.
- `testserv_dns_response_size_bytes{zone, network, family}` - response size in bytes.
