...
const (
	...
	durationMTime                   = -5 * time.Second    // when the signed zone is this much younger, resign
	durationRefreshHours            = 5 * time.Hour       // check zones every 5 hours
	durationRefreshMinutes          = 5 * time.Minute     // check zones every 5 minutes for mtime changes
)
