...
import (
	...
	"github.com/miekg/sndns/plugin/file"
	"github.com/miekg/sndns/plugin/file/tree"
	clog "github.com/miekg/sndns/plugin/pkg/log"
...
func (s *Signer) Sign(now time.Time) (*file.Zone, error) {
	...
	z.Apex.SOA.Minttl = truncateTtl(z.Apex.SOA.Minttl, z.Apex.SOA.Header().Name)
	z.Apex.SOA.Header().Ttl = truncateTtl(z.Apex.SOA.Header().Ttl, z.Apex.SOA.Header().Name)
	...
	err = z.AuthWalk(func(e *tree.Elem, zrrs map[uint16][]dns.RR, auth bool) error {
		...
		for t, rrs := range zrrs {
			// Adjust TTLs of the RRs to not overrun our sign duration
			for j := range rrs {
				rrs[j].Header().Ttl = truncateTtl(rrs[j].Header().Ttl, rrs[j].Header().Name)
			}
...
func (s *Signer) resign(what int) error {
	log.Debugf("Waking up to check signed file for %q", s.origin)
	...
	case checkAll:
		now := time.Now().UTC()
		if err := resign(rd, now); err != nil {
			return err
		}
		fallthrough
	case checkMtime:
		if x := mTime(signedfile).Sub(mTime(s.dbfile)); x < durationMTime {
			return fmt.Errorf("signed file is younger (%s) than zone file", x)
		} else {
			log.Debugf("Signed file is older (%s) than zone file, no need to resign", x)
		}
	}
	return nil
}
...
func (s *Signer) refresh() {
	tickAll := time.NewTicker(durationRefreshHours)
	tickMtime := time.NewTicker(durationRefreshMinutes)
	defer tickAll.Stop()
	defer tickMtime.Stop()
	for {
		select {
		case <-s.stop:
			return
		case <-tickAll.C:
			why := s.resign(checkAll)
			if why == nil {
				continue
			}
			signAndLog(s, why)
		case <-tickMtime.C:
			why := s.resign(checkMtime)
			if why == nil {
				continue
			}
			signAndLog(s, why)
...
// truncateTtl will truncate ttls that are too long, other ttls are returned as-is.
func truncateTtl(t uint32, name string) uint32 {
	if float64(t)*1.2 > durationSignatureExpireDays.Seconds() {
		log.Warningf("NTTL on %q is longer than, or within 20%% of expiration, truncating", name)
		return uint32(durationSignatureExpireDays.Seconds()) / 2
	}
	return t
}
